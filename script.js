...
// Cultura selecionada atualmente para plantar
...
const state = {
  ...
  selectedDrone: null,
  mouseDown: false,
  // Rotas fixas: array de { droneId, fromX, fromY, toRefineryX, toRefineryY }
  routes: [],
  routeMode: false,   // true enquanto o jogador está definindo uma rota
  routeStep: 0,       // 0 = escolhendo drone/origem, 1 = escolhendo destino
...
style.innerHTML = `
  ...
  /* Seletor de cultura */
  .crop-selector { display:flex; gap:6px; margin-bottom:10px; flex-wrap:wrap; }
...
function findClosestRefinery(drone) {
  // Se o drone tem rota fixa, vai pra refinaria da rota
...
function spawnDrone(type) {
  ...
  el.innerHTML = `<span class="battery-bar"><i></i></span><span class="pulse"></span><span class="emoji">${emojis[type]}</span>`
  dronesLayer.appendChild(el)
  const drone = { id, type, x:sx, y:sy, targetX:sx, targetY:sy, battery:100, moveCooldown:0, element:el, task:'idle', patrolTarget:null, carrying:false }
...
function findBestTask(type, drone) {
  ...
  if (type === 'collector') {
    ...
    // Se tem rota fixa: só colhe da origem definida
    if (route) {
...
function pickTargetTile(drone) {
  ...
  // Patrulha aleatória (apenas drones sem rota fixa)
...
function applyTileEffects(tile, drone) {
  ...
  if (type === 'planter') {
    ...
    tile.cropType = selectedCrop
    updateTileVisual(tile)
    log(`Plantio de ${CROPS[selectedCrop].name} em ${getCoordStr(tile.x,tile.y)}`)
    return
  }
  if (type === 'water') {
...
function handleTileClick(e) {
  ...
  // Modo rota: clique define origem do coletor
  if (state.routeMode) {
    handleRouteModeClick(x, y, tileEl)
    return
  }
...
function handleRouteModeClick(x, y, tileEl) {
  if (state.routeStep === 0) {
    // Passo 1: escolher drone coletor (clicando na tile onde ele está)
    const drone = state.drones.find(d => d.type==='collector' && d.x===x && d.y===y)
    if (!drone) {
      // Aceita também clicar em qualquer tile pra definir origem
      const input = prompt(`Defina a tile de ORIGEM da rota (posição a colher). Digite as coords ou confirme a${x+1}b${y+1}:`, getCoordStr(x,y))
      if (!input) return
      ...
      const names = collectors.map((d,i) => `${i+1}: Coletor #${d.id} (pos ${getCoordStr(d.x,d.y)})`).join('\n')
      const choice = prompt(`Qual coletor vai usar nessa rota?\n${names}\nDigite o número:`)
    ...
    log(`🛣️ Coletor definido. Agora clique na refinaria destino.`)

  } else if (state.routeStep === 1) {
    // Passo 2: escolher refinaria destino
    ...
    // Remove rota anterior do mesmo drone
...
function droneLoop() {
  ...
  state.drones.forEach(drone => {
    ...
    // Bateria zerada: vai para carregador
    if (drone.battery <= 0) {
    ...
    if (drone.type==='collector') {
      if (!drone.carrying && target.planted && target.growth>=100) {
        ...
        if (ref) {
          ...
          ref.buffer += 1
          ref._lastCrop = drone._carryingCrop
...
function cropLoop() {
  ...
  state.tiles.forEach(tile => {
    ...
    // Cada cultura tem sua própria velocidade de crescimento
    const growthSpeed = 0.12 * crop.growthRate * weather.growth * waterFactor * luckBonus
    // Soja aguenta mais sem água, milho seca mais rápido
...
function renderUpgradeBars() {
  ['speed','battery','luck'].forEach(type => {
    ...
    if (!bar || !bar.classList.contains('upgrade-bar')) { bar = document.createElement('div'); bar.className='upgrade-bar'; btn.after(bar) }
    bar.innerHTML = Array.from({length:UPGRADE_MAX},(_,i) => `<span class="${i<level?'filled secondary':''}"></span>`).join('')
  })
}
...
function injectCropSelector() {
  ...
  div.id = 'cropSelector'
  div.innerHTML = `
    <p style="font-size:12px;color:#9db39d;margin-bottom:6px">🌿 Cultura a plantar:</p>
    <div class="crop-selector">
      ${Object.entries(CROPS).map(([k,c]) => `
        <button class="crop-btn${k===selectedCrop?' active':''}" data-crop="${k}" title="${c.name}&#10;Vel:${c.growthRate}x  Água:${c.waterNeed}x  Valor:$${c.value}">
          ${c.emoji} ${c.name}
        </button>
      `).join('')}
    </div>
  ...
  div.querySelectorAll('.crop-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      div.querySelectorAll('.crop-btn').forEach(b => b.classList.toggle('active', b.dataset.crop===selectedCrop))
      log(`🌿 Cultura selecionada: ${CROPS[selectedCrop].name}`)
    })
  })
}

// --- INJEÇÃO DE UI DE REFINARIA E ROTA ---
function injectRefineryUI() {
  ...
  btnRoute.style.background = 'linear-gradient(180deg,#ffcc80,#e67e22)'
  btnRoute.style.color = '#1a0d00'
  ...
  btnDelRoute.style.background = 'linear-gradient(180deg,#ff9999,#e74c3c)'
  btnDelRoute.style.color = '#fff'
...
function updateUI() {
  ...
  const elCR = document.getElementById('count-refinery')
  if (elCR) safeText(elCR, state.droneCounts.refinery)
  const elGSD = document.getElementById('gridSizeDisplay')
  if (elGSD) safeText(elGSD, `${state.gridSize}x${state.gridSize}`)
...
function buyDrone(type) {
  ...
  spawnDrone(type); log(`Drone ${type} comprado`); updateUI()
}
...
// --- INIT EVENTS ---
function initEvents() {
  document.querySelectorAll('[data-buy]').forEach(btn => btn.addEventListener('click', () => buyDrone(btn.dataset.buy)))
  ...
  // Clique nas tiles (colheita manual + modo rota)
  if (farmEl) farmEl.addEventListener('click', handleTileClick)

  // Cancelar modo rota com Esc
...
function seedStart() {
  ...
  for (const [x,y,crop] of seeds) {
    ...
    tile.planted=true; tile.growth=rand(12,28); tile.waterBoost=0; tile.pestShield=0; tile.infested=false; tile.cropType=crop
    updateTileVisual(tile)
  }
}
...
function initGame() {
  ...
  log('Fazenda ligada!'); log('Clique em ✅ para colher manualmente!'); log('Use rotas fixas para organizar coletores.')
  updateUI()
}
